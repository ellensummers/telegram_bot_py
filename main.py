...
bot = telebot.TeleBot(token='токен', parse_mode='html')
# Массив с доступными расширениями файлов. Можно изменить, и добавятся/удалятся кнопки выбора на клавиатуре.
formats = ['.jpg', '.png', '.svg', '.gif', '.heic', '.bmp', '.mp4', '.mp3','.avi', '.webm', '.mov', '.doc', '.docx', '.xls', '.xlsx', '.txt', '.pdf', '.pptx', '.json', '.zip', '.rar']
...
def welcome(message):
	...
	markup.add(*formats, row_width=7)

	# Отправляем приветственное сообщение.
	reply = bot.send_message(message.chat.id, f"Привет, <b>{username}</b>! 😊\nСгенерирую тебе файл размером от 1 байта до 45 мегабайтов! \n\nВыбирай расширение из списка ниже ⬇️", reply_markup=markup)
...
def check_unit(message, format):
	...
	# Иначе
		...
		reply = bot.send_message(message.chat.id, f"Ты выбрал/a расширение — <b>{format.text}</b>\nЕдиница измерения — <b>{message.text}</b>\n\nОсталось только указать размер 👌 \nУкажи целое число без пробелов и символов\n⛔️ <u>Ограничения по размеру:</u>\n<b>Минимум</b> — 1 байт\n<b>Максимум</b> — 45 MB (это 46 080 KB или 47 185 920 байт)", reply_markup=markup)
